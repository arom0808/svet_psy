...
class ArticlesList extends Component
{
    ...
    public function render()
    {
        if($this->searchExpression == ""){
            $articles = Article::select("preview_photo_path", "category", "title", "short_description", "publisher_id", "time_to_read", "updated_at")->latest()->paginate($this->limitPerPage);
        }
        else{
            $articles = Article::select("preview_photo_path", "category", "title", "short_description", "publisher_id", "time_to_read", "updated_at")
            ...
            ->orWhere(function ($query) {
                $query->select('name')
                    ->from('users')
                    ->whereColumn('users.id', 'articles.publisher_id');
