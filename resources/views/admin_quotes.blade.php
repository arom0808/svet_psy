<x-app-layout>
    <script>
        function openDeleteModal(quote_id) {
            deleteModal = document.querySelector('#deleteModal');
            deleteModal.style.visibility = "visible";
            deleteModal.action = '/admin/quotes/delete/' + quote_id;
        }
    ...
    <form id="deleteModal" method="POST"
        ...
        @csrf
        <input type="hidden" name="callback" id="callback_input" value="{{ route('admin_quotes') }}" />
                    ...
                    <div class="inline-block min-w-full shadow rounded-lg overflow-hidden">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col"
                                        ...
                                        <a href="{{ route('admin_new_quote') }}">New</a>
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($quotes as $quote)
                                    <tr>
                                        ...
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                            <a href="{{ route('admin_quote', ['id' => $quote->id]) }}"
                                                class="text-indigo-600 hover:text-indigo-900">
                                                Edit
                                            </a>
                                            <button type="button" onclick="openDeleteModal({{ $quote->id }})">
                        ...
                        <div class="px-5 bg-white py-5 flex flex-col xs:flex-row items-center xs:justify-between">
                            <div class="flex items-center">
                                ...
                                @else
                                    <a href='{{ route('admin_quotes') . '?page=' . $page_number - 1 . ($search_expression === '' ? '' : '&search=' . $search_expression) }}'
                                        class="w-full p-4 border text-base rounded-l-xl text-gray-600 bg-white hover:bg-gray-100">
                                        <svg width="9" fill="currentColor" height="8" class="" viewBox="0 0 1792 1792"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
...
@else
                                    <a href='{{ route('admin_quotes') . '?page=' . $page_str . ($search_expression === '' ? '' : '&search=' . $search_expression) }}'
                                ...
                                @else <a
                                        href='{{ route('admin_quotes') . '?page=' . $page_number + 1 . ($search_expression === '' ? '' : '&search=' . $search_expression) }}'
                                        class="w-full p-4 border-t border-b border-r text-base  rounded-r-xl text-gray-600 bg-white hover:bg-gray-100">
                                        <svg width="9" fill="currentColor" height="8" class="" viewBox="0 0 1792 1792"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
