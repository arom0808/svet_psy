    ...
    <div class="bg-white header flex justify-center m-12 flex-col items-center sm:flex-row sm:justify-between">
        ...
        <div class="text-end">
            {{-- <form class="flex w-full max-w-sm space-x-3"> --}}
                <div class=" relative ">
                    <input wire:change="search($event.target.value)" onchange="this.blur(); history.pushState(null, null, '/quotes/'+this.value);" type="text" id="&quot;form-subscribe-Search"
                        class=" rounded-lg border-transparent flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent"
                        value="{{$searchExpression}}" placeholder="Enter" />
                </div>
                {{-- <button
            ...
            <div class="w-full flex flex-col md:flex-row gap-4 mb-8 md:mb-0 flex-between items-center p-8 m-4">
                <div class="bg-white dark:bg-gray-800 w-72 shadow-lg mx-auto rounded-xl p-4">
                    <p class="text-gray-600 dark:text-white">
                        <span class="font-bold text-indigo-500 text-lg">“</span>{{$quotes[$i]->text}}<span class="font-bold text-indigo-500 text-lg">”</span>
                    </p>
